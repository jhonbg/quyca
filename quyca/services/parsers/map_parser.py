...
def get_coauthorship_by_country_map(works):
    ...
    for work in works:
        if not "country_code" in work["affiliation"]["addresses"].keys():
            continue
        if (
            work["affiliation"]["addresses"]["country_code"]
            and work["affiliation"]["addresses"]["country"]
        ):
    ...
    with open(
        os.path.join(os.path.dirname(__file__), "concerns/worldmap.json"), "r"
    ) as worldmap_file:
...
def get_coauthorship_by_colombian_department_map(works: Iterable):
    ...
    with open(
        os.path.join(os.path.dirname(__file__), "concerns/colombiamap.json"), "r"
    ) as colombiamap_file:
