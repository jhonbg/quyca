    ...
    def get_coauthorship_colombia_map(self, data):
        ...
        for work in data:
            ...
            if work["affiliation"]["addresses"]["country_code"] and work["affiliation"]["addresses"]["city"]:
                ...
                department = self.municipios_departamentos[self.municipios_departamentos["MUNICIPIO"] == city][
                    "DEPARTAMENTO"]
                if len(department) == 0:
                    continue
        ...
        for i, feat in enumerate(self.colombiamap["features"]):
            dep_name = feat["properties"]["NOMBRE_DPT"].capitalize() if "bogota" not in feat["properties"][
                "NOMBRE_DPT"].lower() else "Bogotá D.C."
