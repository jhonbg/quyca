...
def test_get_csv_works_by_institution(client):
    ...
    response = client.get(f"/app/affiliation/institution/{random_institution_id}/research/products/csv")
...
def test_get_csv_works_by_department(client):
    ...
    response = client.get(f"/app/affiliation/department/{random_department_id}/research/products/csv")
    assert response.status_code == 200
