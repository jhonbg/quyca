...
def test_get_works_by_institution(client):
    ...
    response = client.get(
        f"/app/affiliation/institution/{random_institution_id}/research/products"
    )
...
def test_get_works_by_faculty(client):
    ...
    response = client.get(
        f"/app/affiliation/faculty/{random_faculty_id}/research/products"
    )
    assert response.status_code == 200
...
def test_get_works_by_department(client):
    ...
    response = client.get(
        f"/app/affiliation/department/{random_department_id}/research/products"
    )
    assert response.status_code == 200
